...
# Load Whisper model (keep your existing implementation)
# whisper_model = whisper.load_model("large")

# Load the new syllable-based disease prediction model
from django.conf import settings
...
#             # Get top predictions (for 3 diseases)
#             top_indices = np.argsort(prediction)[::-1][:3]  # Get all 3 diseases
#             top_predictions = classes[top_indices]
#             top_probabilities = prediction[top_indices] * 100
            
#             # Generate follow-up questions
#             symptom_questions = self.generate_symptom_questions(top_predictions)
...
#                     }
#                     for disease, prob in zip(top_predictions, top_probabilities)
#                 ],
...
#         """Get common symptoms for a disease from database"""
#         disease = Disease.objects.filter(name=disease_name).first()
#         if disease:
#             return list(disease.related_symptoms.values_list('name', flat=True))
#         return []
    
#     def generate_symptom_questions(self, diseases):
...
#         # Calculate which symptoms best differentiate between our 3 diseases
#         symptom_scores = []
#         for symptom in all_symptoms:
#             # Count how many diseases have this symptom
#             positive_count = diseases_qs.filter(related_symptoms=symptom).count()
            
#             # Score based on how well it splits the diseases (1-2 vs others)
#             score = min(positive_count, len(diseases) - positive_count)
...
#         return [{
#             'id': s.id,
#             'text': f"Je, una {s.name.lower()}?",
#             'type': 'boolean'
#         } for s, _ in symptom_scores[:3]]

#     def calculate_probabilities(self, diseases, user_responses, session_id):
#         """Update probabilities based on user responses"""
#         diseases_qs = Disease.objects.filter(name__in=diseases)
#         disease_list = list(diseases_qs)
        
#         # Get the original transcribed text from cache
#         diagnosis_state = cache.get(f'diagnosis_{session_id}')
#         if not diagnosis_state:
#             raise ValueError("Session not found in cache")
        
#         transcribed_text = diagnosis_state['transcribed_text']
        
#         # Start with model's original probabilities
#         original_probs = {d.name: 0 for d in disease_list}
#         for d in disease_list:
#             idx = np.where(disease_model.classes_ == d.name)[0][0]
#             original_probs[d.name] = disease_model.predict_proba([
#                 transcribed_text
#             ])[0][idx] * 100
...
#                 # Adjust probability based on response alignment
#                 if (response and has_symptom) or (not response and not has_symptom):
#                     original_probs[disease.name] *= 1.5  # Increase if matches
#                 else:
#                     original_probs[disease.name] *= 0.7  # Decrease if contradicts
        
#         # Normalize to sum to 100%
#         total = sum(original_probs.values())
#         return {k: (v/total)*100 for k, v in original_probs.items()}
...
#         if sorted_diseases[0][1] > 75.0 or len(diagnosis_state['asked_questions']) >= 5:
#             stage = "conclusion"
#             new_questions = []
#         else:
#             stage = "followup"
#             new_questions = self.generate_symptom_questions(
#                 [d[0] for d in sorted_diseases],
#             )
        
#         # Update state
#         diagnosis_state['current_predictions'] = [d[0] for d in sorted_diseases]
...
#                     "name": name,
#                     "probability": round(prob, 2),
#                     "common_symptoms": self.get_common_symptoms(name)
#                 }
#                 for name, prob in sorted_diseases
#             ],
